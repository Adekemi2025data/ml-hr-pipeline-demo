...
def normalize_column(df, column, method="min-max"):
    ...
    if method == "min-max":
        ...
        raise ValueError("Unknown method. Use 'min-max' or 'z-score'.")
...
def create_age_bins(df, column="Age", bins=None, labels=None):
    """Bin a continuous age column into categories."""
    df = df.copy()
    if column not in df.columns:
        raise ValueError(f"Column '{column}' not found in dataframe")
...
def remove_outliers(df, column, method="iqr", threshold=1.5):
    ...
    if method == "iqr":
        ...
        # Modified z-score (robust for skewed data)
        median = df[column].median()
        mad = np.median(np.abs(df[column] - median))

        if mad == 0:
            return df  # no variation, nothing to remove

        modified_z = 0.6745 * (df[column] - median) / mad
        df = df[abs(modified_z) <= threshold]

    else:
        raise ValueError("Unknown method. Use 'iqr' or 'zscore'.")
