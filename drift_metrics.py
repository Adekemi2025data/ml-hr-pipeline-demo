...
def get_drift_summary(reference, current, label):
    """Run drift detection and return a summary dictionary for HR data."""
    report = Report(metrics=[DataDriftPreset()])
    snapshot = report.run(reference_data=reference, current_data=current)

    result = snapshot.dict()

    # First metric: overall drift summary
    ...
    # Remaining metrics: per-column drift
...
# Analyze all three HR production months
